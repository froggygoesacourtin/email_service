...
def ProcessEmailList(subject, source_email, apikey, template, emailList, selItemDict, UserItemDict, UserItemBlackDict):
   ...
   for email in emailList:
      body = createMail(template, email, selItemDict, UserItemDict[email], UserItemBlackDict[email], maxResultListCount):

      # mail out res
      data = json.dumps({"personalizations": [{"to": [{"email": email}]}],"from": {"email": source_email},"subject": subject,"content": [{"type": "text/plain", "value": body}]})

      c = pycurl.Curl()
      c.setopt(c.URL, "https://api.sendgrid.com/v3/mail/send")
      c.setopt(c.HTTPHEADER, ["Authorization: Bearer %s" apikey),
...
def ProcessEmailFiles(templateFile, emailListFile, selItemFile, UserItemFile, UserItemBlackFile):
   ...
   ProcessEmailList(subject, source_email, apikey, template, emailList, selItemDict, UserItemDict, UserItemBlackDict)
